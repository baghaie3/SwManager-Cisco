...
def scan():
    ...
    for dev in results:
        node_id = dev["ip"]
        node_label = dev.get("name", node_id)  # اگر اسم داشت اسم، اگر نداشت IP
        ...
        for n in dev.get("neighbors", []):
            ...
            sw = Switch.query.filter_by(ip=remote_id).first()
            if sw:
                remote_label = sw.name  
...
@topology_bp.route("/export_visio", methods=["POST"])
@login_required
def export_visio():
    ...
    payload = {
        ...
        "title": data.get("title", "topology_visio"),
        "nodes": nodes,
        "edges": edges,
    }

    response = make_response(jsonify(payload))
    response.headers["Content-Disposition"] = 'attachment; filename=topology_visio.json'
