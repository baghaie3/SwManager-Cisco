...
def remove_mac():
    ...
    if not mac_input or not port or not switch_id:
        ...
        try:
            log_event(
                level="INFO",
                message=f"[PORTSEC] remove-mac requested mac_input={mac_input} port={port} switch_id={switch_id}",
                switch=None,
                status="started",
                file_path="portsec.py",
                event_type="portsec_remove_mac",
            )
        except Exception:
            pass
    ...
    try:
        ...
        # ج) Shutdown پورت
        conn.send_config_set([f"interface {port}", "shutdown"])
        
        # د) ۵ ثانیه صبر (نیاز به import time در بالای فایل دارد)
        import time
        time.sleep(15)
        
        # هـ) No Shutdown پورت
        conn.send_config_set([f"interface {port}", "no shutdown"])

        # --- پایان تغییرات درخواستی ---

        # ذخیره کانفیگ صحیح در IOS
        try:
            conn.save_config()
        except Exception:
            # fallback
        ...
        flash(f"MAC {formatted_mac} removed and port {port} bounced successfully.", "success")
