...
def smb_dashboard():
    # دریافت پارامترهای فیلتر (اضافه شده)
    target_date = request.args.get('date')
    filter_unique = request.args.get('unique_switch') == 'true'

    backups = Backup.query.order_by(
        Backup.created_at.desc()
    ).all()

    # استخراج روزها برای صفحه‌بندی (اضافه شده)
    available_days = sorted(list(set(b.created_at.strftime('%Y-%m-%d') for b in backups if b.created_at)), reverse=True)
    current_day = target_date if target_date else (available_days[0] if available_days else None)

    backup_data = []
    seen_switches = set() # برای فیلتر یکتا (اضافه شده)

    for b in backups:
        # --- بخش فیلترهای جدید ---
        b_date_str = b.created_at.strftime('%Y-%m-%d') if b.created_at else None
        if b_date_str != current_day:
            continue
        
        switch_name = b.switch.name if b.switch else "Unknown"
        if filter_unique:
        ...
        # عیناً کد اصلی شما بدون تغییر حتی یک کاراکتر:
    ...
    return render_template(
        ...
        days=available_days,      # پاس دادن روزها
        current_day=current_day,   # روز فعلی
        unique_filter=filter_unique # وضعیت فیلتر
    )
