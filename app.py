...
executors = {"default": ThreadPoolExecutor(90)}
...
def start_mac_scheduler(app):
    ...
    for sw in Switch.query.all():
        schedule_job(
            ...
            func=scan_switch,
            trigger=IntervalTrigger(minutes=720, timezone=TEHRAN_TZ),
            args=[app, sw.id],
        )
...
def new_switch():
    ...
    if form.validate_on_submit():
        ...
        log_event(
            level="INFO",
            message=f'Switch "{sw.name}" Created',
            switch=sw,
            status="Created",
            file_path="app.py",
            event_type="SwCreate",
...
def edit_switch(id):
    ...
    if form.validate_on_submit():
        ...
        log_event(
            level="INFO",
            message=f'Switch "{sw.name}" updated',
            switch=sw,
            status="Updated",
            file_path="app.py",
            event_type="SwUpdate",
...
def delete_switch(id):
    ...
    log_event(
        level="WARNING",
        message=f'Switch "{sw.name}" Deleted',
        switch=sw,
        status="Deleted",
        file_path="app.py",
        event_type="SwDelete",
...
def new_profile():
    ...
    if form.validate_on_submit():
        ...
        log_event(
            ...
            status="Created",
            file_path="app.py",
            event_type="ProfileCreate",
        )

        flash("✅ New Profile created", "success")
        return redirect(url_for("list_profiles"))

    # حتماً در حالت GET و در حالت فرم نامعتبر به این return برسیم
...
def edit_profile(id):
    ...
    if form.validate_on_submit():
        ...
        log_event(
            level="INFO",
            message=f'Profile "{p.name}" updated',
            status="Updated",
            file_path="app.py",
        ...
        return redirect(url_for("list_profiles"))
    return render_template("edit_profile.html", form=form, profile=p)
...
def delete_profile(id):
    p = CredentialProfile.query.get_or_404(id)
    profile_name = p.name  # برای استفاده بعد از delete
...
def new_location():
    ...
    if form.validate_on_submit():
        ...
        log_event(
            level="INFO",
            message=f'Location "{location.full_path}" created',
            status="Created",
            file_path="app.py",
            event_type="LocationCreate",
        )
...
def edit_location(location_id):
    ...
    if form.validate_on_submit():
        ...
        log_event(
            level="INFO",
            message=f'Location "{location.full_path}" edited',
            status="Edited",
            file_path="app.py",
            event_type="LocationEdit",
        )
...
def delete_location(location_id):
    ...
    log_event(
        level="WARNING",
        message=f'Location "{path}" deleted',
        status="Deleted",
        file_path="app.py",
        event_type="LocationDelete",
    )
